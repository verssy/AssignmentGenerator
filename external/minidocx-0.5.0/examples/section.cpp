...
int main()
{
    ...
    int w, h;
    s2.GetPageSize(w, h);
    std::cout << "Page Size: " << docx::Twip2MM(w) << "mm"
              << " * " << docx::Twip2MM(h) << "mm\n";

    auto orient =
        s2.GetPageOrient() == docx::Section::Orientation::Landscape ? "Landscape" : "Portrait";
    std::cout << "Orientation: " << orient << std::endl;

    s4.SetPageMargin(docx::CM2Twip(2.54), docx::CM2Twip(2.54), docx::CM2Twip(3.175),
                     docx::CM2Twip(3.175));
